// This file contains the updated fetchCanvasAssignments function
// To be manually integrated into main.cpp lines 307-461

int fetchCanvasAssignments() {
  connectWiFi();
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("âŒ WiFi not connected - returning error state");
    consecutiveErrors++;
  ...
  if (!timeSyncComplete) {
    ...
    initTime();
    if (!timeSyncComplete) {
      Serial.println("âŒ Cannot fetch without valid time - returning error state");
  ...
  for (int attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    if (http.begin(client, fullUrl)) {
      ...
      if (httpCode == 200) {
        ...
        if (!error) {
          ...
          if (error.code() == DeserializationError::NoMemory) {
            Serial.println("\n⚠️  NoMemory Error - Attempting Dynamic Buffer Increase");
            ...
            if (newBufferSize <= maxBuffer && newBufferSize > bufferSize) {
              Serial.printf("   ðŸ"„ Increasing buffer: %d â†' %d bytes\n", bufferSize, newBufferSize);
              canvasConfig.jsonBufferSize = newBufferSize;
              
              // Retry with new buffer size
              Serial.println("   â³ Retrying with increased buffer...\n");
              ...
              Serial.println("   âš ï¸ Returning ERROR state - all LEDs will flash");
              http.end();
              consecutiveErrors++;
      ...
      Serial.println("âŒ Could not connect to Canvas");
    }
  ...
  if (consecutiveErrors >= 3) {
    Serial.printf("âš ï¸ %d consecutive Canvas errors - returning ERROR state\n", consecutiveErrors);
    if (consecutiveErrors == 5) {
      Serial.println("ðŸ'¡ TIP: Canvas may be experiencing an outage. All LEDs will flash.");
    }
  }
